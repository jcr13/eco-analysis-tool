...
with open('edge_list_out.dat', 'r') as edges:
    for line_number, line in enumerate(edges, 1):
        ...
        src = row_x[0]
        dst = row_x[1]
        sel_a = 'set sel%sa [atomselect 0 "resid %s and name CA"]' % (int(line_number), int(src))
        sel_b = 'set sel%sb [atomselect 0 "resid %s and name CA"]' % (int(line_number), int(dst))
        ...
        grphx = 'graphics 0 cylinder $crd%sa $crd%sb radius 0.5 filled yes' % (int(line_number), int(line_number))
